...
import type { RefreshTokenRecord, RefreshTokenStore, TransactionRunner } from '@afaghx/afx-core/src/runtime';
...
export class PostgresTransactionRunner implements TransactionRunner {
  ...
  async transaction<T>(work: (tx: TransactionSql) => Promise<T>): Promise<T> {
    return this.db.begin(async (tx) => work(tx));
  }
}

function toIdentity(row: { id: string; subject: string; status: Identity['status']; created_at: Date }): Identity {
...
export class PostgresIdentityStore implements IdentityStore {
  ...
  async findBySubject(subject: string): Promise<Identity | null> {
    const rows = await this.db<[{ id: string; subject: string; status: Identity['status']; created_at: Date }][]>`
...
export class PostgresMembershipStore implements MembershipStore {
  ...
  async resolve(subject: string, tenantId: UUID, organizationId: UUID): Promise<{ membershipId: UUID; roles: string[]; permissions: string[] } | null> {
    const rows = await this.db<[{ id: string; roles: string[] | null; permissions: string[] | null }][]>`
      SELECT m.id, m.roles, m.permissions
      FROM memberships m
    ...
    const row = rows[0];
    if (!row) return null;
    return { membershipId: row.id, roles: row.roles ?? [], permissions: row.permissions ?? [] };
  }
}

export class PostgresSessionRepository implements SessionRepository {
  constructor(private readonly db: Sql | TransactionSql) {}

  async getForUpdate(id: UUID): Promise<Session | null> {
    const rows = await this.db<[{ id: string; identity_id: string; tenant_id: string; refresh_family_id: string; status: Session['status']; created_at: Date; expires_at: Date; last_rotated_at: Date | null }][]>`
...
export class PostgresRefreshTokenRepository implements RefreshTokenRepository {
  ...
  async rotate(input: { current: RefreshTokenRecord; nextHash: string; now: string }): Promise<void> {
    const result = await this.db`
      ...
      SET consumed_at = ${input.now}, revoked_at = NULL
    ...
    await this.db`
      ...
      SELECT gen_random_uuid(), session_id, family_id, ${input.nextHash}, ${input.now}, expires_at
      FROM refresh_tokens
      WHERE token_hash = ${input.current.tokenHash}
    `;
  }

  async revokeFamily(familyId: UUID, reason: string, now: string): Promise<void> {
    await this.db`
      UPDATE refresh_tokens
