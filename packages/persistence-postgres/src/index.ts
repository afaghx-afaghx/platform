...
type SessionRow = { id: string; identity_id: string; tenant_id: string; refresh_family_id: string; status: Session['status']; created_at: Date; expires_at: Date; last_rotated_at: Date | null };
...
export class PostgresRefreshTokenRepository implements RefreshTokenRepository {
  ...
  async findForUpdate(tokenHash: string): Promise<RefreshTokenRecord | null> {
    const rows = await this.db<[{ session_id: string; family_id: string; token_hash: string; consumed_at: Date | null; revoked_at: Date | null; expires_at: Date }][]>`SELECT session_id, family_id, token_hash, consumed_at, revoked_at, expires_at FROM refresh_tokens WHERE token_hash = ${tokenHash} FOR UPDATE`;
    const row = rows[0];
    return row ? { sessionId: row.session_id, familyId: row.family_id, tokenHash: row.token_hash, usedAt: row.consumed_at?.toISOString() ?? null, revokedAt: row.revoked_at?.toISOString() ?? null, expiresAt: row.expires_at.toISOString() } : null;
  }
  async rotate(input: { current: RefreshTokenRecord; nextHash: string; now: string }): Promise<void> {
    const result = await this.db`UPDATE refresh_tokens SET consumed_at = ${input.now} WHERE token_hash = ${input.current.tokenHash} AND consumed_at IS NULL AND revoked_at IS NULL`;
...
export class PostgresIdentityStore implements IdentityStore {
  ...
  async findBySubject(input: { subject: string; tenantId: UUID }): Promise<Identity | null> {
    const rows = await this.db<[{ id: string; subject: string; status: 'active' | 'locked' | 'disabled'; created_at: Date }][]>`SELECT id, subject, status, created_at FROM identities WHERE subject = ${input.subject} AND tenant_id = ${input.tenantId} AND status = 'active' LIMIT 1`;
