...
| G01-10 | Durable DB-backed identity/membership/session state | DONE | AFX-CORE Data | `core/AFX-CORE/` | DB integration + transaction tests + uniqueness test | `db-integration` | Migration + schema + transaction CI artifact | Durable store, unique constraints and atomic refresh rotation |
