...
| G01-14 | MFA foundation | IN PROGRESS | AFX-CORE Identity | `core/AFX-CORE/src/mfa.js`, `core/AFX-CORE/src/persistent-core.js`, `core/AFX-CORE/src/repository.js`, `core/AFX-CORE/test/identity-mfa.test.js` | MFA abuse/recovery tests | `identity-security` | MFA threat/test report + CI artifact | Persistent enrollment, password→MFA challenge gate, replay-safe verification, one-time recovery, expiration/attempt limits and revocation are production tested |
...
G01-10, G01-11 and G01-12 have verified implementation, deterministic automated coverage, dedicated CI execution and reviewable artifacts on the Mission branch. G01-13 remains open pending a corrected calibration assertion and fresh CI evidence. G01-14 implementation is now connected to persistent MFA state and the password authentication flow; dedicated `identity-security` CI is present but its evidence must pass before closure. `G01-20` and `G01-25` remain explicitly BLOCKED until their external/environmental prerequisites exist. Therefore Domain Freeze remains active.
