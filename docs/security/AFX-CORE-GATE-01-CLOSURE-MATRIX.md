...
| G01-10 | Durable DB-backed identity/membership/session state | DONE | AFX-CORE Data | `core/AFX-CORE/migrations/001_g01_10_durable_state.sql` + `core/AFX-CORE/src/repository.js` + `core/AFX-CORE/src/persistent-core.js` | `core/AFX-CORE/test/persistence.test.js` | `security-tests` | CI artifact `afx-core-security-evidence-33861765346` (SHA-256 `3cd38e7ef9460d7e5b6a1c4e5e0cc65264a114e6acd5397ff18de9433088bfc7`) | PostgreSQL-backed state survives restart/multi-instance recreation; migration is idempotent; transaction rollback leaves no partial state |
| G01-11 | HTTP/API Security Boundary | DONE | AFX-CORE API | `core/AFX-CORE/src/http-security-boundary.js` | `core/AFX-CORE/test/http-security-boundary.test.js` | `g01-11-http-api-boundary` | CI artifact `afx-core-g01-11-http-api-boundary-evidence-33861765346` (SHA-256 `3b03785f60c8a6978e9693566baa9b116b2b6bbc10e2048830f63c085f5f7e7b)`, reviewed against successful CI job | CI pass proves authn/authz ordering, 401/403 mapping, protocol/input rejection and no credential/internal-error leakage |
| G01-12 | Concurrency-safe refresh rotation | DONE | AFX-CORE Session | `core/AFX-CORE/` | `test:concurrency` / concurrent refresh race test | `concurrency-security` | CI artifact `afx-core-g01-12-concurrency-evidence-33861765346` (SHA-256 `1eefc692c9bdd1496bcf8ad5fbd1b273124ba8af673687f479b76e0ce2f293cf`) | Concurrent refresh cannot mint multiple valid successors; loser detects reuse and family/session are revoked |
...
G01-10, G01-11 and G01-12 now have executable implementation, deterministic tests, successful GitHub Actions evidence, and recorded artifacts on the hardening branch. This closes the requested persistence, HTTP boundary and refresh-concurrency spine without falsely declaring Gate 01 production-green. G01-13 through G01-26 remain unresolved or blocked as shown below, so Domain Freeze remains active.
