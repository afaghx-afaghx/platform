...
| G01-10 | Durable DB-backed identity/membership/session state | DONE | AFX-CORE Data | `core/AFX-CORE/` | DB integration + transaction tests | `security-tests` | `afx-core-security-evidence-35187284158` artifact | PostgreSQL 16 schema is provisioned; identity/membership/session state survives service-object recreation; session revocation persists; refresh rotation is transactional and concurrent refresh has exactly one winner |
...
The existing bootstrap controls are closed, and G01-10 is now closed based on the latest AFX-CORE Security CI evidence. Production hardening remains incomplete. `G01-20` and `G01-25` are explicitly BLOCKED until their external/environmental prerequisites exist. Therefore Domain Freeze remains active.
