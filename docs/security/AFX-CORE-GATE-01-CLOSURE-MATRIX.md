...
| G01-10 | Durable DB-backed identity/membership/session state | DONE | AFX-CORE Data | `core/AFX-CORE/` | `core/AFX-CORE/test/persistence.test.js` | `security-tests` | PostgreSQL persistence CI artifact | Durable store, service recreation persistence and refresh transaction evidence |
| G01-11 | HTTP/API authentication integration | DONE | AFX-CORE API | `platform/Gateway/runtime.integration.test.mjs` | `platform/Gateway/runtime.integration.test.mjs` | `gateway-security` | Gateway integration CI evidence | Real Gateway → PersistentAfxCore → PostgreSQL path validates auth context |
| G01-12 | Concurrency-safe refresh rotation | DONE | AFX-CORE Session | `core/AFX-CORE/test/persistence.test.js` | `concurrent refresh allows exactly one winner` | `security-tests` | PostgreSQL race-test CI evidence | Concurrent refresh cannot mint multiple valid successors |
...
| G01-26 | Production release security gate | DONE | Platform Security | `.github/workflows/afx-core-gate-01.yml` | closure-matrix gate test | `closure-matrix` | Machine-readable failing/green gate output | Any RED/BLOCKED control prevents protected-branch release |
