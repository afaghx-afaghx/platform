...
| G01-10 | Durable DB-backed identity/membership/session state | DONE | AFX-CORE Data | `core/AFX-CORE/migrations/001_g01_10_durable_state.sql` + `core/AFX-CORE/src/repository.js` + `core/AFX-CORE/src/persistent-core.js` | `core/AFX-CORE/test/persistence.test.js` | `security-tests` | CI artifact `afx-core-security-evidence-33861765346` | PostgreSQL-backed state survives restart/multi-instance recreation; migration is idempotent; transaction rollback leaves no partial state |
| G01-11 | HTTP/API Security Boundary | DONE | AFX-CORE API | `core/AFX-CORE/src/http-security-boundary.js` | `core/AFX-CORE/test/http-security-boundary.test.js` | `g01-11-http-api-boundary` | CI evidence artifact | CI pass proves authn/authz ordering, 401/403 mapping, protocol/input rejection and no credential/internal-error leakage |
| G01-12 | Concurrency-safe refresh rotation | DONE | AFX-CORE Session | `core/AFX-CORE/` | `test:concurrency` / concurrent refresh race test | `concurrency-security` | CI evidence artifact | Concurrent refresh cannot mint multiple valid successors; loser detects reuse and family/session are revoked |
| G01-13 | Production password hashing calibration | DONE | AFX-CORE Security | `core/AFX-CORE/src/security.js` + `docs/security/AFX-CORE-G01-13-PASSWORD-CALIBRATION.md` | `benchmark:password` + security tests | `security-tests` | Run `34154375749` benchmark artifact; calibration record | Calibrated scrypt implementation selected, measured and documented; final Gate review remains required |
...
G01-13 now has executable calibration evidence and a documented engineering decision. G01-14 through G01-26 remain unresolved or blocked as shown below, so Domain Freeze remains active.
