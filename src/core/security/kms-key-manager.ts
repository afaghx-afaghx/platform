...
export class KmsKeyManager {
  ...
  private async loadKeys(): Promise<void> {
    ...
    if (configs.filter((k) => k.status === 'ACTIVE').length !== 1) throw new Error('exactly_one_active_kms_key_required');
    for (const config of configs) {
      ...
      const publicKey = await importSPKI(this.derToPem(result.PublicKey), 'RS256');
  ...
  async sign(signingInput: Uint8Array): Promise<{ kid: string; signature: Uint8Array }> {
    ...
    const result = await this.client.send(new SignCommand({ KeyId: active.keyId, Message: digest, MessageType: 'DIGEST', SigningAlgorithm: 'RSASSA_PKCS1_V1_5_SHA_256' }));
