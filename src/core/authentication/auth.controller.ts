...
export class AuthController {
  ...
  @AfxPublic() @Post('login')
  async login(@Body() body: { email: string; password: string }) {
    const email = body.email.toLowerCase().trim(); const identity = await this.prisma.identity.findUnique({ where: { email } });
    if (!identity || !identity.passwordHash || identity.status !== 'ACTIVE') throw new UnauthorizedException('Invalid credentials');
    try { await this.passwords.verify(identity.passwordHash, body.password); } catch { throw new UnauthorizedException('Invalid credentials'); }
    ...
    if (await this.mfa.hasActiveFactor(identity.id)) {
      ...
      await this.audit.record({ action: 'AUTH.MFA_REQUIRED', subjectId: identity.id, tenantId: membership.tenantId }); return { mfaRequired: true, challengeToken, expiresIn: 300 };
    }
    return this.createSession(identity.id, membership.id, membership.tenantId, membership.organizationId, 'aal1');
  }

  @AfxPublic() @Post('mfa/verify-login')
  async verifyLoginMfa(@Body() body: { challengeToken: string; code: string }) {
    const challenge = await this.auth.verifyMfaChallenge(body.challengeToken); await this.mfa.verifyActiveTotp(challenge.subjectId, body.code);
  ...
  @AfxPublic() @Post('refresh')
  async refresh(@Body() body: { refreshToken: string }) {
    const current = await this.prisma.refreshToken.findUnique({ where: { tokenHash: this.auth.hashRefreshToken(body.refreshToken) }, include: { session: { include: { identity: true } } } });
    ...
    if (current.status !== 'ACTIVE' || current.usedAt) { const now = new Date(); await this.prisma.$transaction([this.prisma.refreshToken.updateMany({ where: { session: { familyId: current.session.familyId } }, data: { status: 'REVOKED', revokedAt: now } }), this.prisma.session.update({ where: { id: current.sessionId }, data: { revokedAt: now } })]); throw new UnauthorizedException('Refresh token reuse detected'); }
    const membership = await this.prisma.membership.findFirst({ where: { identityId: current.session.identityId, status: 'ACTIVE' }, orderBy: { createdAt: 'asc' } }); if (!membership) throw new UnauthorizedException('No active membership');
