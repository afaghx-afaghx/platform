...
export class MfaService {
  ...
  async enrollTotp(identityId: string, label?: string): Promise<{ factorId: string; secret: string; otpauthUri: string }> {
    ...
    `;
    const issuer = process.env.AUTH_MFA_ISSUER ?? 'AFAGHX';
    const account = encodeURIComponent(`${issuer}:${identityId}`);
  ...
  async verifyTotp(identityId: string, factorId: string, code: string): Promise<void> {
    const rows = await this.prisma.$queryRaw<Array<{ id: string; secretCiphertext: string }>>`
      SELECT "id", "secretCiphertext" FROM "MfaFactor"
      WHERE "id"=${factorId}::uuid AND "identityId"=${identityId}::uuid AND "type"='TOTP' AND "status"='PENDING'
      LIMIT 1
    `;
    const row = rows[0];
    if (!row || !/^\d{6}$/.test(code) || !this.validCode(this.decrypt(row.secretCiphertext), code)) {
      throw new UnauthorizedException('Invalid MFA code');
    }
    await this.prisma.$executeRaw`
      UPDATE "MfaFactor" SET "status"='ACTIVE', "verifiedAt"=NOW() WHERE "id"=${factorId}::uuid AND "identityId"=${identityId}::uuid AND "status"='PENDING'
    `;
  }
  ...
  async consumeRecoveryCode(identityId: string, code: string): Promise<void> {
    ...
    const result = await this.prisma.$executeRaw`
      ...
      WHERE "identityId"=${identityId}::uuid AND "codeHash"=${hash} AND "usedAt" IS NULL
    `;
  ...
  async hasActiveFactor(identityId: string): Promise<boolean> {
    const rows = await this.prisma.$queryRaw<Array<{ exists: boolean }>>`
      SELECT EXISTS(SELECT 1 FROM "MfaFactor" WHERE "identityId"=${identityId}::uuid AND "status"='ACTIVE') AS "exists"
    `;
  ...
  private validCode(secret: string, code: string): boolean {
    ...
    for (const offset of [-1, 0, 1]) {
      const candidate = this.totp(secret, counter + offset);
  ...
  private key(): Buffer {
    const configured = process.env.AUTH_MFA_ENCRYPTION_KEY;
    if (!configured) throw new Error('AUTH_MFA_ENCRYPTION_KEY is not configured');
  ...
  private base32Decode(input: string): Buffer {
    ...
    for (const char of input.replace(/=+$/,'').toUpperCase()) { const index = alphabet.indexOf(char); if (index < 0) throw new Error('Invalid base32 secret'); value = (value << 5) | index; bits += 5; if (bits >= 8) { output.push((value >>> (bits - 8)) & 255); bits -= 8; } }
