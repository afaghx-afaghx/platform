(() => {
  ...
  const translations = {
    ...
    '/customer.html': {
      ...
      p2: ['Presentation only · API governed', 'Presentation only · API governed'],
      back: ['بازگشت', 'Back to Experience Hub'],
    },
    '/business.html': {
      ...
      p2: ['Organization-led · API governed', 'Organization-led · API governed'],
      back: ['بازگشت', 'Back to Experience Hub'],
    },
    '/supplier.html': {
    ...
    '/partner.html': {
      ...
      p2: ['Network role · API governed', 'Network role · API governed'],
      back: ['بازگشت', 'Back to Experience Hub'],
    },
  };

  const path = window.location.pathname.replace(/\/index\.html$/, '/index.html');
  const tr = translations[path] || translations['/roles.html'];
  const set = (el, pair) => { if (el) el.textContent = pair[lang() === 'en' ? 1 : 0]; };

  function render() {
    const en = lang() === 'en';
    document.documentElement.lang = en ? 'en' : 'fa';
    document.documentElement.dir = en ? 'ltr' : 'rtl';
    document.title = tr.title[en ? 1 : 0];
    if (path === '/roles.html') {
      set(document.querySelector('main h1'), tr.h1);
      set(document.querySelector('main > p'), tr.p);
      const home = document.querySelector('main > p:last-child a');
      set(home, tr.home);
      document.querySelectorAll('main nav a').forEach((a) => { a.textContent = a.textContent.trim(); });
    } else {
      set(document.querySelector('main p:nth-of-type(1)'), tr.p1);
      set(document.querySelector('main p:nth-of-type(2)'), tr.p2);
      set(document.querySelector('main a'), tr.back);
    }
    ...
    if (!button) {
      ...
      button.addEventListener('click', () => {
        localStorage.setItem('afaghx_lang', en ? 'fa' : 'en');
        render();
      });
    }
    button.textContent = en ? 'FA' : 'EN';
  }
