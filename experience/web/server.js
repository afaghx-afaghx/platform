...
async function bootstrap() {
  ...
  memoryCore.createUser({ email: cfg.email, password: cfg.password });
  const user = memoryCore.users.get(cfg.email.trim().toLowerCase());
...
async function api(req, res, url) {
  ...
  if (req.method === 'POST' && url.pathname === '/api/auth/login') {
    ...
    try {
      ...
      const tokens = await core.authenticatePassword({ email: input.email, password: input.password, tenantId: input.tenantId || config().tenantId });
  ...
  if (req.method === 'POST' && url.pathname === '/api/auth/refresh') {
    try {
      ...
      return authResponse(res, await core.refresh(refresh));
    } catch {
  ...
  if (req.method === 'POST' && url.pathname === '/api/auth/logout') {
    try { (await authContext(req)).sessionId && await core.revokeSession((await authContext(req)).sessionId); } catch { /* idempotent logout */ }
  ...
  const server = http.createServer(async (req, res) => {
    try {
      ...
      if (url.pathname === '/dashboard') {
        try { await authContext(req); } catch { res.writeHead(302, { Location: '/' }); return res.end(); }
      }
      if (!(await staticFile(res, url.pathname))) json(res, 404, { error: 'not_found' }, securityHeaders());
    } catch (error) {
      json(res, error.message === 'payload_too_large' ? 413 : 500, { error: error.message }, securityHeaders());
    }
  });
  return server;
}
