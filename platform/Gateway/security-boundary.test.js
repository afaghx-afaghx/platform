...
test('auth boundary requires bearer token and authorization is tenant-bound', async () => {
  ...
  assert.equal((await boundary.authorize(auth.principal, { tenantId: 't2', permission: 'orders.read' }, async () => true)).status, 403);
  assert.equal((await boundary.authorize(auth.principal, { tenantId: 't1', permission: 'orders.read' }, async () => true)).ok, true);
  assert.equal((await boundary.authorize(auth.principal, { tenantId: 't1', permission: 'orders.write' }, async () => false)).status, 403);
});
