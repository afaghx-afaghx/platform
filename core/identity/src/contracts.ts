...
export interface AuthorizationContext extends TenantContext {
  subject: string;
  roles: string[];
  permissions: string[];
  sessionId: UUID;
...
export interface SessionStore {
  ...
  rotateRefreshToken(sessionId: UUID, presentedTokenHash: string): Promise<{
    session: Session;
...
export interface AuditEvent {
  id: UUID;
  occurredAt: string;
  actor: { subject: string; sessionId?: UUID };
  tenantId?: UUID;
  action: string;
