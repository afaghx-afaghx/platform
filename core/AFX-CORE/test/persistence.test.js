...
test('G01-12 concurrent refresh requests produce exactly one successor and revoke the family on reuse', { skip: !databaseUrl }, async () => {
  ...
  const { rows: familyRows } = await pool.query('SELECT id,current_digest AS "currentDigest",revoked FROM afx_refresh_families WHERE identity_id=$1 AND tenant_id=$2 ORDER BY created_at DESC LIMIT 1', [user.id, 'tenant-race']);
  assert.equal(familyRows.length, 1);
  assert.equal(familyRows[0].revoked, true);
...
test('G01-10 migration is idempotent', { skip: !databaseUrl }, async () => {
  ...
  assert.deepEqual(rows.map(row => row.table_name), ['afx_identities', 'afx_memberships', 'afx_refresh_families', 'afx_refresh_tokens', 'afx_sessions']);
  await pool.end();
});
