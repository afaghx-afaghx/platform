...
const server = createServer(async (req, res) => {
  try {
    if (req.url === '/register/options') {
      const result = webauthn.beginRegistration({ userId: USER_ID, userName: 'browser@afaghx.test', displayName: 'AFAGHX Browser Test' });
      return json(res, 200, result);
    }
    if (req.url === '/register/verify') {
      const body = await readJson(req);
      const result = webauthn.finishRegistration({ userId: USER_ID, challengeId: body.challengeId, credential: body.credential, origin: req.headers.origin });
      return json(res, 200, result);
    }
    if (req.url === '/auth/options') {
      const credentials = webauthn.listCredentials(USER_ID).filter(item => !item.revoked).map(item => item.id);
      const result = webauthn.beginAuthentication({ userId: USER_ID, allowCredentials: credentials });
      return json(res, 200, result);
    }
    if (req.url === '/auth/verify') {
      const body = await readJson(req);
      const result = webauthn.finishAuthentication({ userId: USER_ID, challengeId: body.challengeId, credential: body.credential, origin: req.headers.origin });
      return json(res, 200, result);
    }
    if (req.url === '/evil-register') {
      const body = await readJson(req);
      const result = webauthn.finishRegistration({ userId: USER_ID, challengeId: body.challengeId, credential: body.credential, origin: 'https://evil.example' });
      return json(res, 200, result);
    }
...
function json(res, status, value) { res.writeHead(status, { 'content-type': 'application/json', 'access-control-allow-origin': ORIGIN }); res.end(JSON.stringify(value)); }
...
try {
  ...
  const rejected = await page.evaluate(async ({ credential, challengeId }) => {
    const response = await fetch('/evil-register', { method:'POST', headers:{'content-type':'application/json'}, body: JSON.stringify({ challengeId, credential }) });
    return response.json();
  }, { credential: { id: registered.credentialId, response: {} }, challengeId: 'consumed' });
  assert.equal(rejected.error, 'invalid_registration_response');
  console.log(JSON.stringify({ control:'G01-15', browser:'Chromium', registration:'PASS', authentication:'PASS', rpId:'localhost', origin:ORIGIN, virtualAuthenticator:authenticatorId, invalidOriginBoundary:'PASS' }));
} finally {
  await browser.close();
  server.close();
}
