...
import { decryptMfaSecret, encryptMfaSecret, generateRecoveryCodes, generateTotpCode, generateTotpSecret, hashRecoveryCode, verifyTotpCode, MFA_PARAMETERS } from './mfa.js';
import { WebAuthnService } from './webauthn.js';

export class AfxCore {
  constructor({ clock = () => Date.now(), audit = () => {}, mfaEncryptionKey = null, webauthn = null, recoveryTtlSeconds = 15 * 60 } = {}) {
    this.clock = clock;
    this.audit = audit;
  ...
  beginWebAuthnAuthentication({ userId = null, allowCredentials = [] } = {}) { if (!this.webauthn) throw new Error('webauthn_unavailable'); return this.webauthn.beginAuthentication({ userId, allowCredentials }); }
  finishWebAuthnAuthentication({ userId = null, challengeId, credential, origin }) { if (!this.webauthn) throw new Error('webauthn_unavailable'); const result = this.webauthn.finishAuthentication({ userId, challengeId, credential, origin }); this.audit({ type: 'auth.webauthn.authenticated', userId: result.userId, credentialId: result.credentialId }); return result; }
  ...
  requestPasswordRecovery({ email }) {
    ...
    const now = this.clock();
    for (const [key, value] of this.recoveryTokens) if (value.userId === user.id && !value.used) value.used = true;
    ...
    this._lastRecoveryDelivery = { userId: user.id, email: user.email, token: rawToken };
    return { accepted: true };
  }

  completePasswordRecovery({ recoveryToken, newPassword, mfaRecoveryCode = null }) {
    if (typeof recoveryToken !== 'string' || recoveryToken.length < 20) throw new Error('invalid_recovery_request');
    if (typeof newPassword !== 'string' || newPassword.length < 12) throw new Error('invalid_recovery_request');
    const record = this.recoveryTokens.get(tokenDigest(recoveryToken));
    if (!record || record.used || record.expiresAt <= this.clock()) throw new Error('invalid_recovery_token');
    const user = [...this.users.values()].find(candidate => candidate.id === record.userId);
    if (!user || user.status !== 'active') throw new Error('invalid_recovery_token');
    if (user.mfa?.enabled) {
      if (typeof mfaRecoveryCode !== 'string') throw new Error('mfa_recovery_required');
      const digest = hashRecoveryCode(mfaRecoveryCode);
      const idx = user.mfa.recoveryCodeDigests.indexOf(digest);
      if (idx < 0) throw new Error('mfa_recovery_invalid');
      user.mfa.recoveryCodeDigests.splice(idx, 1);
    }
    record.used = true;
    ...
    this.audit({ type: 'auth.recovery.completed', userId: user.id, mfaProtected: Boolean(user.mfa?.enabled) });
  ...
  refresh(refreshToken) {
    ...
    const digest = tokenDigest(refreshToken), now = this.clock(), tokenRecord = this.refreshTokens.get(digest);
