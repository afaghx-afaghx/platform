...
export class AfxCore {
  constructor({ clock = () => Date.now(), audit = () => {}, mfaEncryptionKey = null, webauthn = null } = {}) {
    this.clock = clock;
    this.audit = audit;
  ...
  authenticatePassword({ email, password, tenantId }) {
    ...
    if (user.mfa?.enabled) {
      ...
      this.mfaChallenges.set(challengeId, { id: challengeId, userId: user.id, tenantId, expiresAt: this.clock() + 5 * 60 * 1000, attempts: 0, consumed: false });
  ...
  confirmMfaEnrollment({ userId, code }) {
    ...
    user.mfa.pending = false; user.mfa.enabled = true; user.mfa.enrolledAt = this.clock();
  ...
  verifyMfaChallenge({ challengeId, code, recoveryCode }) {
    ...
    if (challenge.attempts >= 5) { challenge.consumed = true; throw new Error('mfa_challenge_locked'); }
    ...
    let accepted = false; let recoveryIndex = -1;
    if (typeof code === 'string') { const secret = decryptMfaSecret(user.mfa.secretCiphertext, this.mfaEncryptionKey); accepted = verifyTotpCode(secret, code, this.clock()); }
  ...
  refresh(refreshToken) {
    ...
    const newRefresh = randomToken(), newAccess = randomToken(); tokenRecord.used = true;
    family.currentDigest = tokenDigest(newRefresh); this.refreshTokens.set(family.currentDigest, { familyId: family.id, used: false });
