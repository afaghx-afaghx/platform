export class AfxCoreRepository {
  ...
  async appendAudit() { throw new Error('not_implemented'); }
...
CREATE TABLE IF NOT EXISTS afx_memberships (
  user_id TEXT NOT NULL REFERENCES afx_users(id), tenant_id TEXT NOT NULL,
  roles JSONB NOT NULL DEFAULT '[]'::jsonb, status TEXT NOT NULL CHECK (status IN ('active','disabled')),
...
CREATE TABLE IF NOT EXISTS afx_sessions (
  id TEXT PRIMARY KEY, user_id TEXT NOT NULL REFERENCES afx_users(id), tenant_id TEXT NOT NULL,
  family_id TEXT NOT NULL, access_digest TEXT NOT NULL UNIQUE, access_expires_at TIMESTAMPTZ NOT NULL,
  revoked BOOLEAN NOT NULL DEFAULT false
);
CREATE TABLE IF NOT EXISTS afx_refresh_families (
  id TEXT PRIMARY KEY, user_id TEXT NOT NULL REFERENCES afx_users(id), tenant_id TEXT NOT NULL,
  current_digest TEXT NOT NULL UNIQUE, expires_at TIMESTAMPTZ NOT NULL, revoked BOOLEAN NOT NULL DEFAULT false,
  version BIGINT NOT NULL DEFAULT 0
);
CREATE TABLE IF NOT EXISTS afx_refresh_tokens (
  digest TEXT PRIMARY KEY, family_id TEXT NOT NULL REFERENCES afx_refresh_families(id),
...
CREATE TABLE IF NOT EXISTS afx_security_audit (
  id BIGSERIAL PRIMARY KEY, event_type TEXT NOT NULL, user_id TEXT, tenant_id TEXT, session_id TEXT,
...
CREATE INDEX IF NOT EXISTS afx_security_audit_type_idx ON afx_security_audit(event_type);
`;
...
export class PostgresAfxCoreRepository extends AfxCoreRepository {
  ...
  async rotateRefreshToken({digest,newDigest,newAccessDigest,now,accessExpiresAt}) {
    ...
    try {
      ...
      await client.query('COMMIT'); return family;
    } catch(error){await client.query('ROLLBACK');throw error;} finally{client.release();}
  }
  async revokeRefreshFamily(familyId){const client=await this.pool.connect();try{await client.query('BEGIN');await client.query('UPDATE afx_refresh_families SET revoked=true WHERE id=$1',[familyId]);await client.query('UPDATE afx_sessions SET revoked=true WHERE family_id=$1',[familyId]);await client.query('COMMIT');}catch(e){await client.query('ROLLBACK');throw e;}finally{client.release();}}
  async revokeSession(sessionId){const client=await this.pool.connect();try{await client.query('BEGIN');const {rows}=await client.query('SELECT family_id AS "familyId" FROM afx_sessions WHERE id=$1 FOR UPDATE',[sessionId]);if(!rows[0]){await client.query('COMMIT');return;}await client.query('UPDATE afx_sessions SET revoked=true WHERE id=$1',[sessionId]);await client.query('UPDATE afx_refresh_families SET revoked=true WHERE id=$1',[rows[0].familyId]);await client.query('COMMIT');}catch(e){await client.query('ROLLBACK');throw e;}finally{client.release();}}
