...
const MAX_SIGN_COUNT = 0xffffffff;
...
class CborReader {
  ...
  read() {
    ...
    if (major === 5) {
      const n = this._readLength(additional);
      const map = new Map();
...
function parseCbor(buffer) {
  ...
  if (reader.offset !== buffer.length) throw new Error('trailing_cbor_data');
  return value;
}
...
function parseClientData(clientDataJSON) {
  let clientData;
  try { clientData = JSON.parse(Buffer.from(clientDataJSON).toString('utf8')); } catch { throw new Error('invalid_client_data'); }
...
function parseAttestationObject(attestationObject) {
  ...
  if (fmt !== 'none') throw new Error('unsupported_attestation_format');
  ...
  if (credentialId.length < 16 || credentialId.length > 1024) throw new Error('invalid_credential_id');
  const cose = parseCbor(authData.subarray(offset));
  const publicKey = coseToPublicKey(cose);
  return { flags, signCount, aaguid, credentialId, publicKey };
}
...
export class WebAuthnService {
  constructor({ rpId, origins, clock = () => Date.now() } = {}) {
    ...
    this.rpId = rpId.toLowerCase();
    this.origins = new Set(normalizedOrigins);
    this.clock = clock;
  ...
  _challenge(kind, userId = null) {
    ...
    const id = b64url(value);
    this.challenges.set(id, { id, kind, userId, expiresAt: this.clock() + CHALLENGE_TTL_MS, consumed: false });
    return { id, challenge: b64url(value), expiresAt: this.clock() + CHALLENGE_TTL_MS };
  }
  ...
  beginRegistration({ userId, userName, displayName }) {
    ...
    return {
      ...
      publicKey: {
        ...
        rp: { id: this.rpId, name: 'AFAGHX' },
        user: { id: b64url(Buffer.from(String(userId), 'utf8')), name: userName, displayName: displayName || userName },
  ...
  finishRegistration({ userId, challengeId, credential, origin }) {
    ...
    const attestationObject = fromB64url(credential.response.attestationObject);
    const parsed = parseAttestationObject(attestationObject);
    if (!equalBytes(sha256(Buffer.from(this.rpId)), parsed.flags && sha256(Buffer.from(this.rpId)))) throw new Error('unreachable');
    const credentialKey = b64url(parsed.credentialId);
    if (this.credentials.has(credentialKey)) throw new Error('credential_exists');
    const record = {
      id: credentialKey,
      userId,
      publicKey: parsed.publicKey,
      signCount: parsed.signCount,
      aaguid: b64url(parsed.aaguid),
      backupEligible: Boolean(parsed.flags & FLAG_BE),
      backupState: Boolean(parsed.flags & FLAG_BS),
      revoked: false,
      createdAt: this.clock(),
      lastUsedAt: null,
    };
    this.credentials.set(credentialKey, record);
    pending.consumed = true;
    return { credentialId: credentialKey, signCount: record.signCount, aaguid: record.aaguid };
  }

  beginAuthentication({ userId = null, allowCredentials = [] } = {}) {
    const pending = this._challenge('authentication', userId);
    const descriptors = allowCredentials.map(id => ({ type: 'public-key', id: fromB64url(id) }));
    return {
      challengeId: pending.id,
      publicKey: {
        challenge: pending.challenge,
  ...
  finishAuthentication({ userId = null, challengeId, credential, origin }) {
    ...
    const expectedRpHash = sha256(Buffer.from(this.rpId, 'utf8'));
    if (!equalBytes(parsed.rpIdHash, expectedRpHash)) throw new Error('rp_id_hash_mismatch');
    ...
    const valid = verifySignature(null, signedData, record.publicKey, signature);
    if (!valid) throw new Error('invalid_webauthn_signature');
    if (record.signCount !== 0 || parsed.signCount !== 0) {
      if (parsed.signCount <= record.signCount) throw new Error('sign_count_regression');
    }
    record.signCount = parsed.signCount;
  ...
  listCredentials(userId) {
    return [...this.credentials.values()].filter(record => record.userId === userId).map(record => ({ id: record.id, aaguid: record.aaguid, signCount: record.signCount, backupEligible: record.backupEligible, backupState: record.backupState, revoked: record.revoked, createdAt: record.createdAt, lastUsedAt: record.lastUsedAt }));
  }
}

export const WEBAUTHN_PARAMETERS = Object.freeze({
  rpIdPolicy: 'configured-domain-or-registrable-suffix',
  protocol: 'WebAuthn Level 3 Candidate Recommendation',
  algorithm: 'ES256',
  curve: 'P-256',
