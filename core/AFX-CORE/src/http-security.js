...
export function createHttpSecurityBoundary({ core, authenticateAccessToken, authorize } = {}) {
  ...
  if (typeof authenticate !== 'function') throw new TypeError('authenticateAccessToken must be a function');
  if (typeof authorizeRequest !== 'function') throw new TypeError('authorize must be a function');
  ...
  return async function authenticateAndAuthorize(request, { permission, resourceTenantId } = {}) {
    ...
    let securityContext;
    try { securityContext = await authenticate(token); }
    catch { return { ok: false, status: 401, error: PUBLIC_CREDENTIAL_ERROR }; }
    let allowed = false;
    try { allowed = await authorizeRequest(securityContext, permission, resourceTenantId); }
    catch { return { ok: false, status: 403, error: FORBIDDEN_ERROR }; }
...
export function redactCredential() { return '[REDACTED]'; }
