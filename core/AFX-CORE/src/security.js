...
export function verifyPassword(password, encoded) {
  try {
    ...
    const salt = Buffer.from(salt64, 'base64url'); const expected = Buffer.from(hash64, 'base64url');
    if (salt.length !== SALT_BYTES || expected.length !== KEY_LEN) return false;
    const actual = scryptSync(password, salt, KEY_LEN, { N: SCRYPT_N, r: SCRYPT_R, p: SCRYPT_P, maxmem: MAXMEM_BYTES });
    return timingSafeEqual(expected, actual);
  } catch { return false; }
}
export function sameSecret(a, b) { const left = Buffer.from(a, 'utf8'); const right = Buffer.from(b, 'utf8'); return left.length === right.length && timingSafeEqual(left, right); }
export const SECURITY_PARAMETERS = Object.freeze({ accessTokenTtlSeconds: 300, refreshTokenTtlSeconds: 60 * 60 * 24 * 30, scrypt: { N: SCRYPT_N, r: SCRYPT_R, p: SCRYPT_P, keyLength: KEY_LEN, saltBytes: SALT_BYTES, maxmemBytes: MAXMEM_BYTES }, tokenBytes: TOKEN_BYTES });
