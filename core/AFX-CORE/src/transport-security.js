...
export class SlidingWindowRateLimiter {
  constructor({ limit = 10, windowMs = 60_000, maxKeys = 10_000 } = {}) {
    ...
    if (!Number.isInteger(maxKeys) || maxKeys < 1) throw new TypeError('invalid_max_keys');
    this.limit = limit;
  ...
  consume(key, now = Date.now()) {
    ...
    return {
      ...
      retryAfterSeconds: allowed ? 0 : Math.max(1, Math.ceil(((active[0] ?? now) + this.windowMs - now) / 1000)),
    };
  }
}
...
export function credentialRateLimitKey({ ip = '', email = '', userId = '' } = {}) {
  return createHash('sha256')
    .update(`${ip}|${String(email).trim().toLowerCase()}|${userId}`, 'utf8')
    .digest('base64url');
}
...
export function serializeSessionCookie(value, { maxAgeSeconds = 300, secure = true, sameSite = 'Lax', path = '/' } = {}) {
  ...
  if (path !== '/') throw new Error('invalid_host_cookie_path');
  if (!['Strict', 'Lax', 'None'].includes(sameSite)) throw new Error('invalid_same_site');
  if (sameSite === 'None' && !secure) throw new Error('same_site_none_requires_secure');
  return `__Host-afx_session=${encodeURIComponent(value)}; Max-Age=${Math.floor(maxAgeSeconds)}; Path=/; HttpOnly; Secure; SameSite=${sameSite}`;
}
